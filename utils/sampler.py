...
def sample_function(adj_train, num_nodes, batch_size, n_negative, result_queue):
    ...
    while True:
        for i in range(int(len(user_item_pairs) / batch_size)):
            ...
            # edge_index = torch.tensor(user_positive_items_pairs).t().contiguous()
            # negative_sample_list = list()
            # for _ in range(n_negative):
            #     sample = structured_negative_sampling(edge_index)[-1].tolist()
            #     negative_sample_list.append(sample)
            # user_negative_samples = np.array(negative_sample_list)
            # user_negative_samples = user_negative_samples.reshape(
            #     (user_negative_samples.shape[1], user_negative_samples.shape[0]))
            # print(user_negative_samples)
